...
fn get_pairs_of_sections() ->  utils::MyResult<impl Iterator<Item=(RangeInclusive<u32>,RangeInclusive<u32>)>> {
...
fn day4_1() -> utils::MyResult<()> {
    ...
    let res = pairs_of_sections.filter_map(|(range1, range2)| {
        let fully_contains = |r1: &RangeInclusive<u32>, r2: &RangeInclusive<u32>| r1.start() <= r2.start() && r1.end() >= r2.end();
...
fn day4_2() -> utils::MyResult<()> {
    let pairs_of_sections = get_pairs_of_sections()?;
    let res = pairs_of_sections.filter_map(|(range1, range2)| {
        let overlap = |r1: &RangeInclusive<u32>, r2: &RangeInclusive<u32>| r1.start() <= r2.start() && r1.end() >= r2.start();
